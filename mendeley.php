 ...
 *
 * 	 Commit version: https://github.com/jakob-stoeck/mendeleyapi/commit/70c81443dd19ff7321f5e25c4633e91dcf1f5f98
 */
...
class Mendeley {
	...
	public function __construct($consumerKey = null, $consumerSecret = null) {
		$consumerKey = get_config_plugin('blocktype', 'mendeley', 'key');
		$consumerSecret = get_config_plugin('blocktype', 'mendeley', 'secret');
		require_once(get_config('docroot') . 'blocktype/mendeley/oauth.php');
	...
	*/
	public static function runCurl($url, $method = 'GET', $headers = array(), $postvals = null){
		...
		if($text = MendeleyUtil::isError($http_code)) {
			log_debug(compact('http_code', 'url', 'method'));
			...
			throw new SystemException(sprintf('Error %d (%s): %s', $http_code, $text, $error));
			$response = false;
		}
...
class MendeleyCache {
	...
	public function __construct($suffix = '') {
		$this->dir = get_config('dataroot') . 'sessions/';
		$this->suffix = $suffix;
	}
	...
	public function set($name, $value) {
		...
		if($success == false) {
			throw new SystemException(sprintf('Could not create directory or file: %s. Please check the permissions.', $this->dir . $name . $this->suffix));
		}
	}
	...
	echo '</pre>';
		...
		$success = file_put_contents($dir, $contents) && $success;
